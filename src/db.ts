  ...
  if (search) {
    ...
    return db.prepare(
      `SELECT id, slug, title, artist, key_signature, lang, uploaded_by, version, parent_slug, created_at
       FROM chords_songs WHERE title LIKE ? OR artist LIKE ? ORDER BY created_at DESC LIMIT ? OFFSET ?`
    ).bind(q, q, limit, offset).all<Omit<Song, 'cho_content'>>();
  }
  return db.prepare(
    `SELECT id, slug, title, artist, key_signature, lang, uploaded_by, version, parent_slug, created_at
     FROM chords_songs ORDER BY created_at DESC LIMIT ? OFFSET ?`
  ).bind(limit, offset).all<Omit<Song, 'cho_content'>>();
}

export async function getSong(db: D1Database, slug: string) {
  return db.prepare('SELECT * FROM chords_songs WHERE slug = ?').bind(slug).first<Song>();
}
...
export async function getSongVersions(db: D1Database, slug: string) {
  ...
  return db.prepare(
    `SELECT id, slug, title, artist, version, uploaded_by, created_at
     FROM chords_songs WHERE slug = ? OR parent_slug = ? ORDER BY version`
...
export async function createSong(db: D1Database, song: {
  ...
  return db.prepare(
    `INSERT INTO chords_songs (slug, title, artist, key_signature, lang, cho_content, uploaded_by, tripcode)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
...
export async function createSongVersion(db: D1Database, parentSlug: string, song: {
  ...
  const maxVer = await db.prepare(
    `SELECT MAX(version) as v FROM chords_songs WHERE slug = ? OR parent_slug = ?`
  ).bind(parentSlug, parentSlug).first<{ v: number }>();
  const version = (maxVer?.v || 1) + 1;

  return db.prepare(
    `INSERT INTO chords_songs (slug, title, artist, key_signature, lang, cho_content, uploaded_by, tripcode, version, parent_slug)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(song.slug, song.title, song.artist, song.key_signature || null,
...
export async function getComments(db: D1Database, songSlug: string) {
  const { results } = await db.prepare(
    `SELECT c.*, (SELECT COUNT(*) FROM chords_comments r WHERE r.parent_id = c.id AND r.hidden = 0) as reply_count
     FROM chords_comments c WHERE c.song_slug = ? AND c.parent_id IS NULL AND c.hidden = 0
...
export async function getReplies(db: D1Database, parentId: number) {
  const { results } = await db.prepare(
    'SELECT * FROM chords_comments WHERE parent_id = ? AND hidden = 0 ORDER BY created_at'
...
export async function createComment(db: D1Database, comment: {
  ...
  return db.prepare(
    `INSERT INTO chords_comments (song_slug, username, tripcode, content, parent_id)
     VALUES (?, ?, ?, ?, ?)`
...
export async function reportComment(db: D1Database, id: number) {
  await db.prepare('UPDATE chords_comments SET reports = reports + 1 WHERE id = ?').bind(id).run();
  await db.prepare('UPDATE chords_comments SET hidden = 1 WHERE id = ? AND reports >= 5').bind(id).run();
}

// --- Users ---

export async function getUser(db: D1Database, username: string) {
  return db.prepare('SELECT * FROM chords_users WHERE username = ?').bind(username).first<User>();
}
...
export async function registerUser(db: D1Database, user: {
  ...
  return db.prepare(
    `INSERT INTO chords_users (username, tripcode, color, instrument, bio)
     VALUES (?, ?, ?, ?, ?)
...
export async function follow(db: D1Database, username: string, targetType: string, target: string) {
  return db.prepare(
    'INSERT OR IGNORE INTO chords_follows (username, target_type, target) VALUES (?, ?, ?)'
...
export async function unfollow(db: D1Database, username: string, targetType: string, target: string) {
  return db.prepare(
    'DELETE FROM chords_follows WHERE username = ? AND target_type = ? AND target = ?'
  ).bind(username, targetType, target).run();
}

export async function getFollowing(db: D1Database, username: string) {
  const { results } = await db.prepare(
    'SELECT target_type, target FROM chords_follows WHERE username = ?'
...
export async function getFeed(db: D1Database, username: string, limit = 50) {
  ...
  const { results } = await db.prepare(`
    SELECT 'song' as type, s.slug as id, s.title, s.artist, s.created_at as created_at
    FROM chords_songs s
    INNER JOIN chords_follows f ON f.username = ? AND f.target_type = 'artist' AND f.target = s.artist
    UNION ALL
    SELECT 'comment' as type, c.song_slug as id,
      c.content as title, c.username as artist, c.created_at as created_at
    FROM chords_comments c
    INNER JOIN chords_follows f ON f.username = ? AND f.target_type = 'song' AND f.target = c.song_slug
    WHERE c.hidden = 0
